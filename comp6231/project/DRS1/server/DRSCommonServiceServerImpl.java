...
public class DRSCommonServiceServerImpl extends DRSCommon.DRSCommonServicePOA {
	...
	public DRSCommonServiceServerImpl(String name) {
		...
		} else if (name.equals(SvrInfo.SVR_NAME_VANCOUVER)) {
			port = SvrInfo.SVR_PORT_VANCOUVER;
		} else if (name.equals(SvrInfo.SVR2_NAME_MONTREAL)) {
			port = SvrInfo.SVR2_PORT_MONTREAL;
		} else if (name.equals(SvrInfo.SVR2_NAME_TORONTO)) {
			port = SvrInfo.SVR2_PORT_TORONTO;
		} else if (name.equals(SvrInfo.SVR2_NAME_VANCOUVER)) {
			port = SvrInfo.SVR2_PORT_VANCOUVER;
		} else if (name.equals(SvrInfo.SVR3_NAME_MONTREAL)) {
			port = SvrInfo.SVR3_PORT_MONTREAL;
		} else if (name.equals(SvrInfo.SVR3_NAME_TORONTO)) {
			port = SvrInfo.SVR3_PORT_TORONTO;
		} else if (name.equals(SvrInfo.SVR3_NAME_VANCOUVER)) {
			port = SvrInfo.SVR3_PORT_VANCOUVER;
		}
		svr.init(name, port);
		svrName = name;
	}
