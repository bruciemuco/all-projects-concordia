 ...
 */
@Path("torontoport")
public class TorontoPort {
    private WS_client1.Toronto port;

    @Context
    private UriInfo context;

    /**
     * Creates a new instance of TorontoPort
     */
    public TorontoPort() {
        port = getPort();
    }
     ...
     *
     */
    private WS_client1.Toronto getPort() {
        try {
            // Call Web Service Operation
            WS_client1.Toronto_Service service = new WS_client1.Toronto_Service();
            WS_client1.Toronto p = service.getTorontoPort();
            return p;
        } catch (Exception ex) {
            // TODO handle custom exceptions here
        }
