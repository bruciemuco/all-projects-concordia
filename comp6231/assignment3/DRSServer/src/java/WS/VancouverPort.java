 ...
 */
@Path("vancouverport")
public class VancouverPort {
    private WS_client2.Vancouver port;

    @Context
    private UriInfo context;

    /**
     * Creates a new instance of VancouverPort
     */
    public VancouverPort() {
        port = getPort();
    }
     ...
     *
     */
    private WS_client2.Vancouver getPort() {
        try {
            // Call Web Service Operation
            WS_client2.Vancouver_Service service = new WS_client2.Vancouver_Service();
            WS_client2.Vancouver p = service.getVancouverPort();
            return p;
        } catch (Exception ex) {
            // TODO handle custom exceptions here
        }
